...
<body>
	...
	<c:set var="questno" value="${param.questno }" />
	<%-- mav.addObject("questno", questionVO.getQuestno()); // redirect parameter 적용 --%>
	<c:set var="fcateno" value="${param.fcateno }" />
	<%-- mav.addObject("fcateno", questionVO.getFcateno()); // redirect parameter 적용 --%>
			...
			<ul>
				<c:choose>
					...
					<c:when test="${code == 'create_success'}">
						<%-- Java if --%>
						<li class='li_none'><span class="span_success">새로운 질문을 등록했습니다.</span></li>
					</c:when>
					...
					<c:when test="${code == 'create_fail'}">
						<%-- Java if --%>
						<LI class='li_none'><span class="span_fail">새로운 질문 등록에 실패했습니다.</span></LI>
					</c:when>

					<c:when test="${code == 'update_fail'}">
						<%-- Java if --%>
						<LI class='li_none'><span class="span_fail">질문 수정에 실패했습니다.</span></LI>
					</c:when>

					<c:when test="${code == 'delete_success'}">
						<%-- Java if --%>
						<LI class='li_none'><span class="span_success">질문 삭제에 성공했습니다.</span></LI>
					</c:when>

					<c:when test="${code == 'delete_fail'}">
						<%-- Java if --%>
						<LI class='li_none'><span class="span_fail">질문 삭제에 실패했습니다.</span></LI>
					</c:when>
