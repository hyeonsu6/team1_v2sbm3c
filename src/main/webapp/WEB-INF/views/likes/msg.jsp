...
<c:set var="fcateno" value="${param.fcateno }" /> <%-- mav.addObject("fcateno", contentsVO.getCateno()); // redirect parameter 적용 --%>
    ...
    <UL>
      <c:choose>
        ...
        <c:when test="${code == 'create_success'}"> <%-- Java if --%>
          <li class='li_none'>
            <span class="span_success">좋아요 등록했습니다.</span>
          </li> 
        </c:when>
        ...
        <c:when test="${code == 'create_fail'}"> <%-- Java if --%>
          <LI class='li_none'>
            <span class="span_fail">좋아요 등록에 실패했습니다.</span>
          </LI>                                                                      
        </c:when>
        
        <c:when test="${code == 'update_fail'}"> <%-- Java if --%>
          <LI class='li_none'>
            <span class="span_fail">좋아요 수정에 실패했습니다.</span>
          </LI>                                                                      
        </c:when>
        
        <c:when test="${code == 'delete_success'}"> <%-- Java if --%>
          <LI class='li_none'>
            <span class="span_success">좋아요 삭제에 성공했습니다.</span>
          </LI>                                                                      
        </c:when>        
        
        <c:when test="${code == 'delete_fail'}"> <%-- Java if --%>
          <LI class='li_none'>
            <span class="span_fail">좋아요 삭제에 실패했습니다.</span>
          </LI>                                                                      
        </c:when> 
