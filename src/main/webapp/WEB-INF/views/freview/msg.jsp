...
<title>Festival Blog Festival Review Message</title>
...
<body>
	...
	<div class='title_line'>페스티벌 콘텐츠 리뷰 알림</div>
			...
			<ul>
				<c:choose>
					...
					<c:when test="${code == 'create_success'}">
						<%-- Java if --%>
						<li class='li_none'><span class="span_success">새로운 리뷰를 등록했습니다.</span></li>
					</c:when>
					...
					<c:when test="${code == 'create_fail'}">
						<%-- Java if --%>
						<li class='li_none'><span class="span_fail">새로운 리뷰 등록에 실패했습니다.</span></li>
					</c:when>

					<c:when test="${code == 'update_fail'}">
						<%-- Java if --%>
						<li class='li_none'><span class="span_fail">리뷰 수정에 실패했습니다.</span></li>
					</c:when>

					<c:when test="${code == 'delete_success'}">
						<%-- Java if --%>
						<li class='li_none'><span class="span_success">리뷰 삭제에 성공했습니다.</span></li>
					</c:when>

					<c:when test="${code == 'delete_fail'}">
						<%-- Java if --%>
						<li class='li_none'><span class="span_fail">리뷰 삭제에 실패했습니다.</span></li>
					</c:when>
				...
				<li class='li_none'><br> <c:choose>
					...
					<button type='button' onclick="location.href='./list_by_contentsno.do?contentsno=${param.contentsno}'"
						class="btn btn-outline-warning btn-sm" style="background-color: #B8860B;">리뷰 목록</button></li>
			</ul>
		</fieldset>
	</div>
