...
public class QuestionCont {
   ...
   * 목록 + 검색 + 페이징 지원
   * 검색하지 않는 경우
   * http://localhost:9093/question/list_by_fcateno.do?fcateno=2&now_page=1
   * 검색하는 경우
  ...
  public ModelAndView list_by_fcateno(QuestionVO questionVO) {
     ...
     */
    String paging = questionProc.pagingBox(questionVO.getFcateno(), questionVO.getNow_page(), "list_by_fcateno.do", search_count);
  ...
  public ModelAndView list_by_fcateno_grid(QuestionVO questionVO) {
     ...
     * @return 페이징용으로 생성된 HTML/CSS tag 문자열
     */
    String paging = questionProc.pagingBox(questionVO.getFcateno(), questionVO.getNow_page(), "list_by_fcateno_grid.do", search_count);
    mav.addObject("paging", paging);
  ...
  public ModelAndView read(HttpSession session, int questno) {
    ...
    AnswerVO answerVO = answerProc.read_questno(questno);
    if(answerVO != null) {
