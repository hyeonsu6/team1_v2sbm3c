...
//	    request.setAttribute("fcateVO", fcateVO);
	...
	public ModelAndView create(HttpServletRequest request, HttpSession session, FestivalVO festivalVO) {
		...
		if (adminProc.isAdmin(session)) { // 관리자로 로그인한경우
			...
			if (Tool.checkUploadFile(file1) == true) { // 업로드 가능한 파일인지 검사
				...
				festivalVO.setFile1(file1); // 순수 원본 파일명
				festivalVO.setFile1saved(file1saved); // 저장된 파일명(파일명 중복 처리)
				festivalVO.setThumb1(thumb1); // 원본이미지 축소판
				festivalVO.setSize1(size1); // 파일 크기
				// ------------------------------------------------------------------------------
				// 파일 전송 코드 종료
				// ------------------------------------------------------------------------------
				...
				// PK의 return
				// ------------------------------------------------------------------------------
				// System.out.println("--> contentsno: " + festivalVO.getContentsno());
				// mav.addObject("contentsno", festivalVO.getContentsno()); // redirect
				// parameter 적용
				// ------------------------------------------------------------------------------

				if (cnt == 1) {
					mav.addObject("code", "create_success");
					// fcateProc.increaseCnt(festivalVO.getFcateno()); // 글수 증가
				} else {
				...
				mav.addObject("fcateno", festivalVO.getFcateno()); // redirect parameter 적용
	...
	// /**
	// * 특정 카테고리의 검색 목록
	// * http://localhost:9093/festival/list_by_fcateno.do?fcateno=1
	// * @return
	// */
	// @RequestMapping(value="/festival/list_by_fcateno.do", method =
	// RequestMethod.GET)
...
//	    hashMap.put("word", word);
	//
//	    ArrayList<FestivalVO> list = this.festivalProc.list_by_cateno_search(hashMap);
	//
//	    // for문을 사용하여 객체를 추출, Call By Reference 기반의 원본 객체 값 변경
//	    for (FestivalVO festivalVO : list) {
//	      String title = festivalVO.getTitle();
...
//	    return mav;
	...
	public ModelAndView list_by_fcateno(FestivalVO festivalVO) {
		...
		int search_count = this.festivalProc.search_count(hashMap); // 검색된 레코드 갯수 -> 전체 페이지 규모 파악
	...
	public ModelAndView map(int contentsno) {
		...
		FestivalVO festivalVO = this.festivalProc.read(contentsno); // map 정보 읽어 오기
	...
	public ModelAndView update_text(HttpSession session, FestivalVO festivalVO) {
		...
		mav.addObject("now_page", festivalVO.getNow_page()); // POST -> GET: 데이터 분실이 발생함으로 다시한번 데이터 저장 ★

		// URL에 파라미터의 전송
		// mav.setViewName("redirect:/festivalVO/read.do?contentsno=" +
	...
	public ModelAndView update_file(HttpSession session, FestivalVO festivalVO) {
		...
		if (this.adminProc.isAdmin(session)) {
			...
			// 파일 삭제 시작
			// -------------------------------------------------------------------
			String file1saved = festivalVO_old.getFile1saved(); // 실제 저장된 파일명
			String thumb1 = festivalVO_old.getThumb1(); // 실제 저장된 preview 이미지 파일명
			long size1 = 0;
			...
			this.festivalProc.update_file(festivalVO); // Oracle 처리
	...
	public ModelAndView delete(FestivalVO festivalVO) {
		...
		this.festivalProc.delete(festivalVO.getContentsno()); // DBMS 삭제
	...
	/**
	 * http://localhost:9093/festival/delete_by_fcateno.do?fcateno=1 파일 삭제 -> 레코드 삭제
	 * 
	 * @param fcateno
	 * @return
	 */
	...
	public ModelAndView list_all_gallery(HttpSession session) {
		...
		if (this.adminProc.isAdmin(session) == true) {
			mav.setViewName("/festival/list_all_gallery"); // /WEB-INF/views/festival/list_all_gallery.jsp
