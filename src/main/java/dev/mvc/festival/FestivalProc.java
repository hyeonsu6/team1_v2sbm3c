...
public class FestivalProc implements FestivalProcInter {
	...
	public ArrayList<FestivalVO> list_by_fcateno_search_paging(FestivalVO festivalVO) {
		 ...
		 */
		int begin_of_page = (festivalVO.getNow_page() - 1) * Festival.RECORD_PER_PAGE;
...
//  <span class='span_box_1'><A href='list_by_fcateno.do?word=&now_page=3&fcateno=4'>3</A></span>
//    </DIV>
//  </DIV> 
//  <!-- 페이지 목록 출력 부분 종료 -->
	...
	public String pagingBox(int fcateno, int now_page, String word, String list_file, int search_count) {
		...
		int total_page = (int) (Math.ceil((double) search_count / Festival.RECORD_PER_PAGE));
		// 전체 그룹 수: (double)1/10 -> 0.1 -> 1 그룹, (double)12/10 -> 1.2 그룹-> 2 그룹
		int total_grp = (int) (Math.ceil((double) total_page / Festival.PAGE_PER_BLOCK));
		// 현재 그룹 번호: (double)13/10 -> 1.3 -> 2 그룹
		int now_grp = (int) (Math.ceil((double) now_page / Festival.PAGE_PER_BLOCK));

		// 1 group: 1, 2, 3 ... 9, 10
		// 2 group: 11, 12 ... 19, 20
		// 3 group: 21, 22 ... 29, 30
		int start_page = ((now_grp - 1) * Festival.PAGE_PER_BLOCK) + 1; // 특정 그룹의 시작 페이지
		int end_page = (now_grp * Festival.PAGE_PER_BLOCK); // 특정 그룹의 마지막 페이지
...
//      str.append("현재 페이지: " + nowPage + " / " + totalPage + "  "); 
		...
		_now_page = (now_grp * Festival.PAGE_PER_BLOCK) + 1; // 최대 페이지수 + 1
		if (now_grp < total_grp) {
			str.append("<span class='span_box_1'><A href='" + list_file + "?&word=" + word + "&now_page=" + _now_page
					+ "&fcateno=" + fcateno + "'>다음</A></span>");
		}
		str.append("</DIV>");
