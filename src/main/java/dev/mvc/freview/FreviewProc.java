...
public class FreviewProc implements FreviewProcInter {
	...
	private FreviewDAOInter freviewDAO;

	@Override
	public int create(FreviewVO freviewVO) {
		// 등록
		int cnt = this.freviewDAO.create(freviewVO);
		return cnt;
	}

	@Override
	public ArrayList<FreviewVO> list_all() {
		ArrayList<FreviewVO> list = this.freviewDAO.list_all();
		return list;
	}

	@Override
	public ArrayList<FreviewVO> list_by_contentsno(int contentsno) {
		ArrayList<FreviewVO> list = this.freviewDAO.list_by_contentsno(contentsno);
		return list;
	}
	...
	public FreviewVO read(int reviewno) {
		FreviewVO freviewVO = this.freviewDAO.read(reviewno);
		return freviewVO;
	}

	@Override
	public int map(HashMap<String, Object> map) {
		int cnt = this.freviewDAO.map(map);
		return cnt;
	}

	@Override
	public ArrayList<FreviewVO> list_by_contentsno_search(HashMap<String, Object> hashMap) {
		ArrayList<FreviewVO> list = this.freviewDAO.list_by_contentsno_search(hashMap);
		return list;
	}

	@Override
	public int search_count(HashMap<String, Object> hashMap) {
		int cnt = this.freviewDAO.search_count(hashMap);
		return cnt;
	}
	...
	public ArrayList<FreviewVO> list_by_contentsno_search_paging(FreviewVO freviewVO) {
		 ...
		 */
		int begin_of_page = (freviewVO.getNow_page() - 1) * Freview.RECORD_PER_PAGE;
		...
		freviewVO.setStart_num(start_num);
		freviewVO.setEnd_num(end_num);

		ArrayList<FreviewVO> list = this.freviewDAO.list_by_contentsno_search_paging(freviewVO);
	...
	// #paging A:hover{text-decoration:none; background-color: #FFFFFF; color:black;
	// font-size: 1em;}
...
//	    text-align: center;    
	...
	//
	// <DIV id='paging'>
//	    <span class='span_box_1'><A href='list_by_fcateno.do?word=&now_page=1&fcateno=4'>1</A></span>
	// <span class='span_box_2'>2</span>
	// <span class='span_box_1'><A
	// href='list_by_fcateno.do?word=&now_page=3&fcateno=4'>3</A></span>
//	    </DIV>
	// </DIV>
	// <!-- 페이지 목록 출력 부분 종료 -->
	...
	public String pagingBox(int contentsno, int now_page, String word, String list_file, int search_count) {
		...
		int total_page = (int) (Math.ceil((double) search_count / Freview.RECORD_PER_PAGE));
		// 전체 그룹 수: (double)1/10 -> 0.1 -> 1 그룹, (double)12/10 -> 1.2 그룹-> 2 그룹
		int total_grp = (int) (Math.ceil((double) total_page / Freview.PAGE_PER_BLOCK));
		// 현재 그룹 번호: (double)13/10 -> 1.3 -> 2 그룹
		int now_grp = (int) (Math.ceil((double) now_page / Freview.PAGE_PER_BLOCK));

		// 1 group: 1, 2, 3 ... 9, 10
		// 2 group: 11, 12 ... 19, 20
		// 3 group: 21, 22 ... 29, 30
		int start_page = ((now_grp - 1) * Freview.PAGE_PER_BLOCK) + 1; // 특정 그룹의 시작 페이지
		int end_page = (now_grp * Freview.PAGE_PER_BLOCK); // 특정 그룹의 마지막 페이지
		...
		str.append("    border-color: #2F4F4F;");
		str.append("    padding:1px 6px 1px 6px; /*위, 오른쪽, 아래, 왼쪽*/");
		str.append("    margin:1px 2px 1px 2px; /*위, 오른쪽, 아래, 왼쪽*/");
		str.append("    border-radius: 10px");
		str.append("  }");
		str.append("  .span_box_2{");
		str.append("    text-align: center;");
		str.append("    background-color: #2F4F4F;");
...
//	      str.append("현재 페이지: " + nowPage + " / " + totalPage + "  "); 
		...
		for (int i = start_page; i <= end_page; i++) {
			...
			if (now_page == i) { // 목록에 출력하는 페이지가 현재페이지와 같다면 CSS 강조(차별을 둠)
				...
				str.append("<span class='span_box_1'><A href='" + list_file + "?word=" + word + "&now_page=" + i
						+ "&contentsno=" + contentsno + "'>" + i + "</A></span>");
			}
		}
		...
		_now_page = (now_grp * Freview.PAGE_PER_BLOCK) + 1; // 최대 페이지수 + 1
		if (now_grp < total_grp) {
			str.append("<span class='span_box_1'><A href='" + list_file + "?&word=" + word + "&now_page=" + _now_page
					+ "&contentsno=" + contentsno + "'>다음</A></span>");
		}
		str.append("</DIV>");
	...
	public int count_by_contentsno(int contentsno) {
		int cnt = this.freviewDAO.count_by_contentsno(contentsno);
		return cnt;
	}
